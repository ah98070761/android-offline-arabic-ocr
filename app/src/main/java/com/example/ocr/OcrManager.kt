...
class OcrManager(private val context: Context) {
    ...
    suspend fun performOcr(imageUri: Uri): String = withContext(Dispatchers.IO) {
        try {
            ...
            if (text.isBlank()) "⚠️ لم يتم العثور على أي نص في الصورة." else text
        } catch (e: IOException) {
    ...
    suspend fun performOcrOnPdf(pdfUri: Uri): String = withContext(Dispatchers.IO) {
        ...
        try {
            parcelFileDescriptor = context.contentResolver.openFileDescriptor(pdfUri, "r") ?: return@withContext "❌ فشل فتح ملف PDF."
